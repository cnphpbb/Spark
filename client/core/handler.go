...
func ping(pack modules.Packet, wsConn *common.Conn) {
	_ = wsConn.SendCallback(modules.Packet{Code: 0}, pack)
	device, err := GetPartialInfo()
	if err != nil {
		golog.Error(err)
		return
	}
	_ = wsConn.SendPack(modules.CommonPack{Act: `DEVICE_UPDATE`, Data: *device})
}
...
func lock(pack modules.Packet, wsConn *common.Conn) {
	err := basic.Lock()
	if err != nil {
		_ = wsConn.SendCallback(modules.Packet{Code: 1, Msg: err.Error()}, pack)
	} else {
		_ = wsConn.SendCallback(modules.Packet{Code: 0}, pack)
	}
}
...
func screenshot(pack modules.Packet, wsConn *common.Conn) {
	...
	if val, ok := pack.GetData(`bridge`, reflect.String); !ok {
		_ = wsConn.SendCallback(modules.Packet{Code: 1, Msg: `${i18n|COMMON.INVALID_PARAMETER}`}, pack)
		return
	} else {
		bridge = val.(string)
	}
...
func initTerminal(pack modules.Packet, wsConn *common.Conn) {
	...
	if err != nil {
		_ = wsConn.SendCallback(modules.Packet{Act: `TERMINAL_INIT`, Code: 1, Msg: err.Error()}, pack)
	} else {
		_ = wsConn.SendCallback(modules.Packet{Act: `TERMINAL_INIT`, Code: 0}, pack)
	}
}
...
func listFiles(pack modules.Packet, wsConn *common.Conn) {
	...
	if err != nil {
		...
		_ = wsConn.SendCallback(modules.Packet{Code: 0, Data: smap{`files`: files}}, pack)
	}
}
...
func fetchFile(pack modules.Packet, wsConn *common.Conn) {
	...
	if val, ok := pack.GetData(`path`, reflect.String); !ok {
		_ = wsConn.SendCallback(modules.Packet{Code: 1, Msg: `${i18n|EXPLORER.FILE_OR_DIR_NOT_EXIST}`}, pack)
		return
	} else {
		path = val.(string)
	}
...
func listProcesses(pack modules.Packet, wsConn *common.Conn) {
	...
	if err != nil {
		...
		_ = wsConn.SendCallback(modules.Packet{Code: 0, Data: map[string]any{`processes`: processes}}, pack)
	}
}
...
func initDesktop(pack modules.Packet, wsConn *common.Conn) {
	...
	if err != nil {
		_ = wsConn.SendCallback(modules.Packet{Act: `DESKTOP_INIT`, Code: 1, Msg: err.Error()}, pack)
	} else {
		_ = wsConn.SendCallback(modules.Packet{Act: `DESKTOP_INIT`, Code: 0}, pack)
	}
}
...
func execCommand(pack modules.Packet, wsConn *common.Conn) {
	...
	if err != nil {
		...
		_ = wsConn.SendCallback(modules.Packet{Code: 0, Data: map[string]any{
